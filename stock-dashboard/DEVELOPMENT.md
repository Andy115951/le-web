...
## 当前已推送、待部署批次（2026-09-03）

这一批改动已作为 `c8f6878 feat(stock): harden research workflow and calendar` 推送到 `origin/main`，但**尚未部署或写入 Supabase**。生产仍运行旧版 `research-quality-v3`，不应把下面的本地 UI 与接口能力当成已经上线。跨设备继续前先运行：
...
本批次的外部数据边界：不得在未确认前运行带 `--approve` 的派生重建或财报导入；NDX 新快照仍需有完整的 Nasdaq 官方来源与人工差异审核。部署前必须重新运行 `npm test`，并在需要线上验证时使用只读接口；部署本身不会重建派生数据或导入财报候选。
...
- 前端「研究回放」区已展示已验证叙述（标题/复盘/逐条 claim 引用/不确定性），按快照 `packet_fingerprint` 匹配已接受叙述。`2026-08-31` 修复引用计数漏算 `baselineKeys`，并把「N 条引用」升级为分类证据标签：事件（蓝）/行情基准（绿）/相似日（紫），让每条 claim 引的证据类型可读、可追溯。前端不提供模型调用按钮，只读展示服务端已通过校验的叙述。
