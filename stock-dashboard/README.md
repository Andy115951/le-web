...
2. `deepseek-v3.2` 与 `deepseek-v4-flash` 无项目数据探针均已通过。`2026-08-24` 已对 `2026-08-21` 研究快照用 `deepseek-v4-flash` 完成一次受控验证（`accepted`）。新人解读的 AI 润色必须是单独确认按钮，不能绑在「读一下」上；网页仍不得自动出站
