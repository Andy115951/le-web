...
- 个人决策日志：在观察记录上一键起草，记录已做的买入/加仓/减仓/清仓/继续持有/观望/放弃、理由和事后结果；删除以最小 tombstone 跨设备同步，旧设备不会复活已删记录；不生成买卖建议
...
- 个人决策日志：`decision-logs.mjs` 校验/归一/last-write-wins 合并、补录结果与最小删除 tombstone；`watchlist_states.decision_logs` 加性列继承 RLS；看板可从观察记录一键起草，只记录用户已做决定
...
- `QQQ` 五年日线首次回填验证曾有 1,254 个唯一交易日，覆盖 `2021-08-12` 至 `2026-08-11`；当前最新日期以研究覆盖面板为准
...
- 已公布结果的时间展示：有官方精确发布时间时显示“官方发布”及双时区；缺失该证据时只保留盘前/盘后或时段未知，不使用系统采集时间替代
...
- 日历休市口径：2026–2028 年 NYSE 已公布的全天休市日单独标记为“NYSE 全天休市”；周末、未来日期和未知工作日行情缺失保持分离，提前收盘仍视为交易日
...
- 受控派生维护：`npm run research:rebuild-derived` 默认只显示 QQQ 的重建计划；只有追加 `-- --approve` 才会依次写入特征、前瞻标签与相似日，且不会由网页或 Cron 自动触发
- 特征查询接口：`GET /api/nasdaq/features?symbol=QQQ&limit=365`，支持 `date=YYYY-MM-DD` 精确读取
- 历史相似日基线：按目标日之前的价格、风险、成交活跃度和已知事件计数匹配非连续历史阶段，并展示每个候选的只读维度分数与已成熟后续表现
...
- 无项目数据网关探针：单独使用固定 JSON 验证 `finish_reason=stop` 与 JSON 模式；JSON 请求关闭 V4 thinking、不走 stream；服务端先预留一次性审计记录；默认关闭且不触碰研究快照。`2026-08-24` 已分别对 `deepseek-v3.2` 与当前生产模型 `deepseek-v4-flash` 各打通一次（均为 `accepted`）；同一模型名下额度已用完
...
- 新人关联解读：默认只有说明和「读一下今天 / 读一下这一天」；点出五段模板后，可另点「生成 AI 解读」（卡片内小框确认）。讲解出现在下面单独区域，不替换五段事实；仅发送页面可见事实段，不发送持仓数量和成本；服务端拒绝空状态复述、前瞻走势判断与操作建议。失败或未开启时保留模板。独立开关 `DEEPSEEK_BEGINNER_READING_ENABLED`，不打开收盘研究复盘
...
2. `deepseek-v3.2` 与 `deepseek-v4-flash` 无项目数据探针均已通过。`2026-08-24` 已对 `2026-08-21` 研究快照用 `deepseek-v4-flash` 完成一次受控验证（`accepted`）；已接受的研究摘要已按快照指纹接入研究回放。新人解读 AI 走单独按钮「生成 AI 解读」，结果写在模板下方的讲解区；生产已打开 `DEEPSEEK_BEGINNER_READING_ENABLED`，不打开收盘研究复盘
...
- `decision-review.mjs`: 决策日志的只读复盘统计
...
- 已提供 `manifest.webmanifest`、应用图标和 `sw.js`。在 HTTPS 的生产地址或 localhost 上，支持原生安装提示的浏览器会显示“安装应用”按钮。
- iPhone/iPad 没有该原生提示时，可在 Safari 的分享菜单选择“添加到主屏幕”。
- Service Worker 只缓存成功返回的 HTML 页面外壳和静态代码；离线壳清单受测试约束，覆盖页面的全部本地静态模块。离线时最多打开已缓存的界面；行情、新闻、Supabase 会话、持仓、决策日志和所有 `/api`、`/auth` 请求从不写入 PWA 缓存，恢复网络后仍须刷新获取实时数据。
- PWA 不替代云同步。跨设备的个人数据仍只通过登录后的 Supabase RLS 数据同步。
...
2. 决策日志层
   ...
   - 已完成：基础个人复盘统计（总记录、待复盘、已补录结果、动作和标的分布），以及新日志/补录结果的私有价格与持仓快照；已补录且前后快照完整时会显示价格、股数和成本的事实对照，只聚合用户手动记录，不评价策略
   - 边界：旧日志不会回填或猜测历史上下文；前后价格变化不等于决策结果，结果标签仍完全由用户补录
...
5. AI 研究摘要层
   ...
   - 已完成：完整流程回放按快照关联采集、日报、已验证模型摘要与到期审计；网页不触发模型调用
   - 后续：仅在维护者重新批准后考虑受控的定时研究摘要运行；不自动生成交易指令
