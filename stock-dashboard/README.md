...
- 阶段重试与耗时：研究输入、日报、冻结周报和到期评估遇到短暂网络故障时最多自动重试一次；每次尝试都追加记录安全状态、排队和运行耗时，原始异常不进入接口或网页
...
- 无项目数据网关探针：单独使用固定 JSON 验证 `finish_reason=stop` 与 JSON 模式，服务端先预留一次性审计记录；默认关闭且不触碰研究快照
