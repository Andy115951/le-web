...
- `decision_logs`: 用户手写的决策日志。记录你在某标的上的实际动作（买入/加仓/减仓/清仓/继续持有/观望/放弃）、理由，以及事后结果复盘；可关联到某条自动观察记录。新日志和补录结果会在同一私有 JSON 内冻结当时可用的价格、峰值、成本、股数和持仓类型。已补录且两端快照完整时，页面仅并列展示价格、股数和成本的事实变化；旧日志不会回填，价格变化也不自动解释为决策好坏。只保存你已经做过的决定，系统不生成任何买卖建议。跨设备同步按 `updatedAt` 最后写入优先，最多保留 500 条。
