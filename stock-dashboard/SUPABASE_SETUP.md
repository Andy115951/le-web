...
输入包会主动排除：目标日未来 1/3/5/20 日标签、目标日美东收盘后才可获得的事件、交易建议、目标价和任何模型概率。事件窗口从上一实际交易日收盘后开始，到目标日美东收盘结束，因此前一日收盘后公布的信息会在下一日研究包中出现，不会被丢弃。事件摘要也会从过滤后的事件列表重新计算，避免收盘后的数量或影响等级间接泄漏。相似日的后续收益仅属于更早候选日的已成熟历史结果，且会保留方法与归一化信息。
