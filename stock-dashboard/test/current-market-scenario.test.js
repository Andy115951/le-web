...
} = require("../lib/current-market-scenario");
...
test("current market scenario exposes a bounded empirical summary without raw candidates", function () {
  ...
  assert.equal(scenario.reading.version, CURRENT_MARKET_SCENARIO_READING_VERSION);
  assert.match(scenario.reading.headline, /2026-08-11/);
  assert.equal(scenario.reading.observations.length, 3);
  assert.match(scenario.reading.caution, /不是当前预测/);
...
test("current market scenario remains explicit when no current target or mature samples exist", function () {
  const awaitingTarget = buildCurrentMarketScenario({ summary: {} });
  const insufficientSamples = buildCurrentMarketScenario({ target: { market_date: "2026-08-11" }, summary: { candidateCount: 0 } });
  assert.equal(awaitingTarget.status, "awaiting_target");
  assert.match(awaitingTarget.reading.caution, /没有用缺失数据/);
  assert.equal(insufficientSamples.status, "insufficient_samples");
  assert.match(insufficientSamples.reading.caution, /不补零/);
});
