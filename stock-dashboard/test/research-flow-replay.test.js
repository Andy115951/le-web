...
const { buildResearchFlowReplay, getResearchFlowReplay, normalizeSnapshotId } = require("../lib/research-flow-replay");
...
test("research flow replay queries exact snapshot and fingerprint keys", async function () {
  ...
  const flow = await getResearchFlowReplay({ snapshotId: SNAPSHOT_ID }, { url: "https://example.invalid" }, async function (_config, path) {
    paths.push(path);
    if (path.includes("research_packet_snapshots")) return [{ id: SNAPSHOT_ID, packet_fingerprint: FINGERPRINT }];
    return [];
  });
