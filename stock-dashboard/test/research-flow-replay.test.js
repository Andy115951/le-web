...
test("research flow replays only safe stage summaries from an exactly linked capture run", function () {
  const flow = buildResearchFlowReplay({
    ...
    captureTasks: [
      { task_kind: "daily_fact_report", task_version: "daily-research-report-v1", status: "succeeded", created_at: "2026-08-12T00:01:00.000Z", details: { raw: "must-not-appear" } },
  ...
  assert.deepEqual(summarizeCaptureTasks([{ task_kind: "market_collection", status: "partial" }]), {
    market_collection: { status: "partial", taskVersion: null, createdAt: null }
  });
});
...
test("research flow replay queries exact snapshot and fingerprint keys", async function () {
  ...
  assert.equal(paths.some(function (path) { return path.includes("research_task_runs") && path.includes("capture_run_id=eq.999e4567-e89b-12d3-a456-426614174000") && path.includes("select=task_kind,task_version,status,created_at"); }), true);
