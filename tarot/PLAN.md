# Tarot Plan（冻结 v2.23 · P44 首次来访引导 · 2026-09-15）

> 实现状态：P0–P44 已落地（大阿尔卡纳全套 + 圣杯 + 权杖 + 宝剑 + 星币花色混合位图齐；78 张 `CARD_ART`；追问子牌阵 + 象征牌链；牌阵剧场三阵；烛火信物；场景剧场软提示；同题回看对照；凯尔特十字十位；GitHub/Google OAuth；静默模式；易用性：折叠高级选项 / 快速起卦 / 骨架屏 / 仪式略加速；解读 Markdown 渲染；场景微剧本开场/落烛；分享/信物预览保存与系统分享拆分；克制多局记忆：同题旧卦轻提；PWA 主屏安装 + 弱网壳缓存）。AI 默认 mock；`AI_PROVIDER=deepseek`（legacy `gateway`）对齐 stock-dashboard OpenAI 兼容接口，不再用 Vercel AI Gateway。生产需 `DEEPSEEK_API_KEY`。Vercel↔GitHub 自动部署已接通。混合位图渐进已收官（不强制外购整副）。**P43 真机浸泡清单已落地**（`SOAK.md` + 小刺修复）；**P44 首次来访引导已落地**；**P45–P51 体验队列已冻结方向、待实现**（近端磨顺 → 中段顾问感；见 §3.47–§3.53 / §5）。产品决策仍按下表冻结。
...
体验抛光队列（见 §5）：**P43–P44 已完成**；**P45–P46 近端磨顺**（访客登录转化 / 历史好找）；**P47–P51 中段顾问感**（语气校准 / 局内时间线 / 克制多局记忆加一层 / 公开分享页 / 今日一牌习惯）。
...
见 `DEV.md`。P43–P44 已完成；P45–P51 方向已定、实现待做；付费/手机号等仍见 DEV「待续讨论」。
