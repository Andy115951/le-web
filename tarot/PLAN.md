# Tarot Plan（冻结 v0.8 · 2026-09-10）

> 实现状态：P0–P6 已落地（牌库、仪式动画、解读/追问、历史、额度与登录、示意卡面）。AI 默认 mock，gateway 已接线（`AI_PROVIDER=gateway`）。剩余主要为可选流式 UI、精美写实卡面、GitHub 自动部署。产品决策仍按下表冻结。
...
| 技术（实现向） | Next.js App Router + Tailwind + **shadcn**（基础控件）+ AI SDK（gateway / mock）；仪式/牌面自定义；认证对齐 quadrant-todo |
...
含：场景起卦、文本仪式（三速）、简要/详细解读、自由追问、新占卜按钮与软提示、历史卡片、访客/登录额度、用户名密码登录、深色烛光 UI、简化牌面示意、AI mock + gateway 接线。

不含（v1）：付费、OAuth/手机号、精美写实卡面、流式解读 UI、凯尔特十字等复杂牌阵、社交分享精修图。
