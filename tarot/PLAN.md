# Tarot Plan（冻结 v2.21 · 体验队列 P43–P51 · 2026-09-15）

> 实现状态：P0–P42 已落地（大阿尔卡纳全套 + 圣杯 + 权杖 + 宝剑 + 星币花色混合位图齐；78 张 `CARD_ART`；追问子牌阵 + 象征牌链；牌阵剧场三阵；烛火信物；场景剧场软提示；同题回看对照；凯尔特十字十位；GitHub/Google OAuth；静默模式；易用性：折叠高级选项 / 快速起卦 / 骨架屏 / 仪式略加速；解读 Markdown 渲染；场景微剧本开场/落烛；分享/信物预览保存与系统分享拆分；克制多局记忆：同题旧卦轻提；PWA 主屏安装 + 弱网壳缓存）。AI 默认 mock；`AI_PROVIDER=deepseek`（legacy `gateway`）对齐 stock-dashboard OpenAI 兼容接口，不再用 Vercel AI Gateway。生产需 `DEEPSEEK_API_KEY`。Vercel↔GitHub 自动部署已接通。混合位图渐进已收官（不强制外购整副）。**下一批体验队列 P43–P51 已冻结方向、待实现**（近端磨顺 → 中段顾问感；见 §3.45–§3.53 / §5）。产品决策仍按下表冻结。
...
- 示例问题、场景语气、启动/空状态微文案、子牌阵/剧场软提示/信物按钮态、快速起卦见同目录 `COPY.md`（v1.15）
...
### 3.45 P43 真机浸泡清单（已定方向 · 待做）

- 整条真机路径走通：起卦 → 仪式 → 解读 → 追问 → 分享 → 主屏安装
- 专修卡顿、文案硌、按钮藏太深等小刺；**不新开大功能**
- 产出可勾选浸泡清单（文档或看板备注即可）；修一项合并一项

### 3.46 P44 首次来访引导（已定方向 · 待做）

- 第一次进站：用 **1～2 句**烛光旁白讲清「怎么起一卦」
- **不做**教程墙、多步 wizard、强制遮罩
- 看过一次后不再打扰（localStorage 或等价即可）

### 3.47 P45 访客→登录转化（已定方向 · 待做）

- 额度将近用尽 / 用尽时：文案更温柔、路径更短（少跳转、登录 CTA 就近）
- GitHub OAuth 已有；**Google OAuth 仍可选补上**（需人工配 env，不阻塞本项文案/路径）
- 不改额度数字本身（除非另开需求）

### 3.48 P46 历史好找（已定方向 · 待做）

- 历史列表：按 **场景 / 时间**筛选
- **同题成组**（`normalizeQuestion` 对齐 P32/P41）
- **软收藏**（可星标/钉住；勿做成复杂标签系统）
- 目标：历史一长仍能快速找回

### 3.49 P47 解读语气再校准（已定方向 · 待做）

- 六场景口吻更分明（感情更柔、事业更稳、日常抉择更短平等，对齐既有场景语气表）
- Markdown 结构更稳：优先 **总览 / 牌意 / 综合**；减少飘移小节
- 仍禁绝对预言；对外文案禁「塔罗」

### 3.50 P48 局内时间线（已定方向 · 待做）

- 本卦轻量时间轴：揭晓 → 解读 → 追问/象征牌 → 信物
- 偏回看导航，不打断仪式；移动端可折叠/侧栏轻量
- 无强制新 DB；能用现有 messages / 状态推导则优先

### 3.51 P49 多局记忆加一层（仍克制）（已定方向 · 待做）

- 在 P41 同题轻提之上：若近几日问过**相关主题**（非必须同句），开篇可再轻提 **一句**
- **仍不**注入旧 AI 全文；**不**做成跨局长聊天 / 通用 agent 记忆
- 可关或默认克制；无则行为与今相同

### 3.52 P50 公开分享页（已定方向 · 待做）

- 可选只读短链：朋友打开可见牌阵摘要 + 娱乐免责（比纯图片更完整）
- 用户可关 / 可失效链接；不含账号、cookie、内部 id、AI 原文全文（默认）
- 文案禁「塔罗」
...
| P43 | 真机浸泡清单（小刺修复，不大开功能） | 待做 |
| P44 | 首次来访引导（1～2 句旁白） | 待做 |
| P45 | 访客→登录转化（更温柔短路径） | 待做 |
| P46 | 历史好找（场景/时间筛选、同题成组、软收藏） | 待做 |
| P47 | 解读语气再校准（场景口吻 + Markdown 结构） | 待做 |
| P48 | 局内时间线（揭晓→解读→追问→信物） | 待做 |
| P49 | 多局记忆加一层（近几日相关主题轻提，仍克制） | 待做 |
| P50 | 公开分享页（只读短链，可关） | 待做 |
| P51 | 今日一牌习惯（轻提醒 / 主屏默认落点） | 待做 |

## 6. 详细设计

见 `DEV.md`。体验队列 P43–P51 方向已定、实现待做；付费/手机号等仍见 DEV「待续讨论」。
