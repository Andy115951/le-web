...
- 示例问题、场景语气、启动/空状态微文案、子牌阵/剧场软提示/信物按钮态、快速起卦见同目录 `COPY.md`（v1.23）
...
- **冻结**：不要求外购整副牌面；`CARD_ART` 已齐（大阿尔卡纳 + 四花色，78 张），后续仅按需重修个别牌面
...
- 混合 webp 桥接见 P17；`CARD_ART` 78 张已齐；**冻结：不要求外购整副写实牌面**（与 §3.10 / DEV 一致）
...
- 文字「分享牌阵」路径保留；混合位图桥接见 P17（78 张已齐，分享 PNG 同桥）
...
- `CARD_ART`（`src/data/card-art.ts`）**只列已有文件**；大阿尔卡纳 `major_00`–`major_21` 已齐；小阿尔卡纳四花色已齐（78 张）
...
- 六场景口吻更分明（感情更柔、事业更稳、日常抉择更短平快，对齐 COPY「场景语气」；`scenes.ts` `tonePrompt` + prompts 权重「必须可辨认」）
...
| P17 | 混合卡面桥接（webp + glyph 回退；CARD_ART 由 P19–P25 收官至 78） | 已完成 |
...
| P51 | 今日一牌习惯（轻提醒 / 主屏默认落点） | 已完成 |

## 6. 详细设计

见 `DEV.md`。P43–P51 体验队列已完成；付费/手机号等仍见 DEV「待续讨论」。
