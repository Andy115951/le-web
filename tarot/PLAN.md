...
- 在 P41 同题轻提之上：若近几日（约 7 天）问过**相关主题**（非必须同句），开篇可再轻提 **一句**
- 相关判定：同非自定义场景，或问题软 token 重叠（`related-theme.ts`）；复用 store `findRelatedThemeReading`；注入 `relatedThemeHint`（场景/问题摘要/牌阵/牌面正逆）
- **仍不**注入旧 AI 全文；**不**做成跨局长聊天 / 通用 agent 记忆；无相关旧卦时行为不变；追问不变；无 migration
- 默认克制：仅解读开篇；与同题 `priorHint` 可并存，合计仍两句内
...
体验抛光队列（见 §5）：**P43–P49 已完成**；**P50–P51 中段顾问感**（公开分享页 / 今日一牌习惯）。
...
见 `DEV.md`。P43–P49 已完成；P50–P51 方向已定、实现待做；付费/手机号等仍见 DEV「待续讨论」。
