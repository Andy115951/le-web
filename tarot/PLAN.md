# Tarot Plan（冻结 v2.9 · 2026-09-11）

> 实现状态：P0–P30 已落地（大阿尔卡纳全套 + 圣杯 + 权杖 + 宝剑 + 星币花色混合位图齐；78 张 `CARD_ART`；追问子牌阵；牌阵剧场三阵；烛火信物）。AI 默认 mock；`AI_PROVIDER=deepseek`（legacy `gateway`）对齐 stock-dashboard OpenAI 兼容接口，不再用 Vercel AI Gateway。生产需 `DEEPSEEK_API_KEY`。Vercel↔GitHub 自动部署已接通。混合位图渐进已收官（不强制外购整副）。产品决策仍按下表冻结。
...
含：场景起卦、文本仪式（三速）、简要/详细解读、自由追问（含追问子牌阵）、新占卜按钮与软提示、历史卡片（重命名/软删）、访客/登录额度、用户名密码登录、深色烛光 UI、插画风牌面示意 + 大阿尔卡纳混合位图全套 + 圣杯/权杖花色混合位图、AI mock + deepseek 接线、流式解读/追问 UI、基础无障碍与空状态抛光、分享牌阵纯文本摘要、烛光分享 PNG、烛火信物竖版壁纸、额度感知 UX、牌义图鉴、加厚牌库释义、首页今日一牌、牌阵剧场三阵（关系双人/抉择分叉/月相三问）。
