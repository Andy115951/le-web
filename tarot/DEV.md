...
| P37 易用性 | 起卦高级选项默认折叠「牌阵与仪式（可选）」；首页 `QuickStartButton` 一键起卦；`loading.tsx` 骨架（new/history；`[id]` 已撤以免 `notFound` 被 200 盖住）；Link prefetch；仪式 1200/650/280ms，无跳过 |
